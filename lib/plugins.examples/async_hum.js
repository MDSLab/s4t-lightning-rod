...
var BaseAsyncPlugin = require('../process/plugin/baseasync');
var PluginAPI = require('../process/plugin/api');
...
HumPlugin.prototype._compute = function () {
    ...
    return Q.spread([board.readAnalog(this._pinTemp), board.readAnalog(this._pinHum)],
        function (temp_volt, HIH4030_Value) {
            ...
            return [PluginAPI.sendToCKAN(this._mAuthid, this._mResourceid, record), relativeHumidity, temp];

        }.bind(this))
        .spread(function (payloadJSON, relativeHumidity, temp) {
            var results = "Humidity " + relativeHumidity + " percent (with " + temp + " °C) sent to CKAN";

...
var shared = require('./shared/socat');
...
function message(logmsg, status) {
    return {name: shared.name, status: status, logmsg: logmsg}
}
...
function SocatWrapper(basePort, socatBoardIp, socatServerIp) {
    ...
    socatProcess.stderr.on('data', function (data) {
        ...
        greIfaceProcess.on('close', function (code) {
            ...
            greIfaceUpProcess.on('close', function (code) {
                ...
                process.send(message("tunnels configured", shared.messageStatusChoices.complete));
            });
        });
    });
    ...
    //NEW-net
    process.send(message("I'm alive!", shared.messageStatusChoices.alive));
}

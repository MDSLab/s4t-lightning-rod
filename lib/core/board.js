...
function BoardPosition(position) {
    if (!position
        ...
        // logger.warn("Unable to load the board position");
...
BoardState.prototype.reload = function (nconf_) {
    ...
    this._localServices = nconf_.get('config:board:services');
    ...
    // todo save state !!!!!
    // todo validate state
    this._boardCode = nconf_.get('config:board:code');

    this._position = new BoardPosition(nconf_.get('config:board:position'));

    var reg_status = nconf_.get('config:board:status');
...
function _initDeviceClass() {
    ...
    try {
        return require('./device/' + this._device);
    }
    catch (e) {

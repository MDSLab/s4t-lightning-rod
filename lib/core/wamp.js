...
function _wampStateConnectionOnOpen() {
    ...
    if (boardStatus === Board.getState().REGISTRATION_STATUSES.NEW) {
        ...
        provisioningChain = this._wampConnection.getSession().call("s4t.board.provisioning", [boardCode])
            ...
            .then(function() {
                return Board.getState().setRegistrationStatus(Board.getState().REGISTRATION_STATUSES.REGISTERED);
            })
    ...
    provisioningChain
        .then(function() {
            ...
            this._wampConnection.getSession().publish(connectionTopic, [boardCode, 'connection', this._wampConnection.getSession().getId()]);
        }.bind(this))
        .done();

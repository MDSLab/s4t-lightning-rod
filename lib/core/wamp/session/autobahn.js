...
var AutobahnRegistration = require('../registration/autobahn');
 ...
 * @param endpoint
 * @param options
 * @returns {Q.Promise} AutobahnRegistration
 */
AutobahnSession.prototype.register = function (procedure, endpoint, options) {
    return this._session.register(procedure, endpoint, options)
        .then(function (registration) {
            return new AutobahnRegistration(registration);
        });
};


/**
 * Unregisters previously registered procedure
 * @param registration {AutobahnRegistration}
 * @returns {Q.Promise}
 */
AutobahnSession.prototype.unregister = function (registration) {
    return this._session.unregister(registration.registration);
};
